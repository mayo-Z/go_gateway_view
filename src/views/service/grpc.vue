    ...
    <el-row>
      <el-card class="box-card">
        <div slot="header" class="clearfix">
          <span v-if="isEdit == true">创建GRPC服务</span>
          <span v-if="isEdit == false">修改GRPC服务</span>
        </div>
          ...
          <el-form ref="form" :model="sizeForm" label-width="140px" size="mini">
            ...
            <el-form-item label="metadata转换">
              <el-input
                ...
                placeholder="metadata转换支持增加(add)、删除(del)、修改(edit) 格式: add headname headvalue 多条换行"
                type="textarea"
      ...
      serviceDetail(query)
        .then((response) => {
          ...
          this.sizeForm.header_transfor =
            formData.grpc_rule.header_transfor.replace(/,/g, "\n");
          ...
          this.sizeForm.port = formData.grpc_rule.port.toString();
        })
        .catch(() => {});
    },
    handleSubmit() {
